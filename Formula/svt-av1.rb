class SvtAv1 < Formula
  desc "AV1 encoder library (shared) - fork with sensible defaults"
  homepage "https://github.com/nekotrix/SVT-AV1-Essential"
  url "https://github.com/nekotrix/SVT-AV1-Essential/archive/refs/tags/v4.0.1-Essential.tar.gz"
  sha256 "4575b5b50eb6888f358d235e44c6caf1a968812ce2200692b2025e5ea082244b"
  license "BSD-2-Clause"

  livecheck do
    url :stable
    strategy :github_latest
  end

  bottle do
    root_url "https://github.com/fraluc06/homebrew-ffmpeg-svt-av1-essential/releases/download/svt-av1-4.0.1"
    sha256 cellar: :any, arm64_tahoe:  "d34202473ca7e6e8a6c81048589bd8669f5f1df1c958e719b917315851005176"
    sha256 cellar: :any, x86_64_linux: "48b1005a7ff1f251f290008fcacb28c6a0f020bb090e32bd1b88e632a9f46587"
  end

  depends_on "cmake" => :build
  depends_on "nasm" => :build
  depends_on "pkgconf" => :build

  def install
    args = %W[
      -DCMAKE_BUILD_TYPE=Release
      -DCMAKE_INSTALL_PREFIX=#{prefix}
      -DCMAKE_INSTALL_LIBDIR=lib
      -DCMAKE_INSTALL_INCLUDEDIR=include
      -DBUILD_SHARED_LIBS=ON
      -DBUILD_APPS=OFF
      -DBUILD_DEC=OFF
      -DSVT_AV1_LTO=ON
      -DENABLE_AVX512=OFF
      -DENABLE_NEON_I8MM=OFF
      -DREPRODUCIBLE_BUILDS=ON
      -DUSE_WEBM_IO=OFF
    ]

    system "cmake", "-S", ".", "-B", "svt_build", *args
    system "cmake", "--build", "svt_build", "--config", "Release", "--parallel"
    system "cmake", "--install", "svt_build"
  end

  def caveats
    <<~EOS
      SVT-AV1-Essential library (shared) has been installed.
      This library is used by ffmpeg for AV1 encoding.
    EOS
  end

  test do
    (testpath/"test.c").write <<~EOS
      #include <svt-av1/EbSvtAv1Enc.h>
      int main() {
        EbSvtAv1EncConfiguration config;
        return 0;
      }
    EOS
    system ENV.cc, "test.c", "-I#{include}", "-L#{lib}", "-lSvtAv1Enc", "-o", "test"
    system "./test"
  end
end
